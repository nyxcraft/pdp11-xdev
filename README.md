...
| `ultrix1` `ultrix2` `ultrix3` `ultrix31` | full | DEC Ultrix-11 1.0 (= V7M-11 1.0, 1982/83), 2.0 (1984), 3.0 (1986, which merged 4.2BSD TCP/IP as an Executive-mode driver) and 3.1 (1987, the last release) — DEC's productized Version 7 for the PDP-11. The local 3.1 `sysent.c` is authoritative: V7 core numbers (1–48) plus DEC/SysV additions (`msgsys`/`utssys`/`fcntl`/`ulimit`/`nap`… at 49+, no user-visible sockets — 3.0/3.1 share the user interface), `0407`/`0410`/`0411` a.out, apsim's Ultrix personality. **Native-validated**: real V7M-11 1.0 `/bin` (`echo`/`cat`/`wc`/`sum`/`ls`/`od`…), carved from the `agn453/V7M-11` disk image into `~/unix/ultrix11/1.0`, runs under `-u ultrix1` and byte-identically across all four, `wc`/`sum` matching the host. The 2.0 install tape carves too (134 native `/bin` from a UNIX `dump`), and those run under `-u ultrix2` — they use Ultrix's stack-argument syscall variant (`sys N|0200`), which apsim decodes; `wc`/`sum` byte-match the host. (Old names `ultrix11v1`/`ultrix11v2`/`ultrix11` remain as aliases.) |
