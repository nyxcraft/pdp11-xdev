...
| `ultrix1` `ultrix2` `ultrix3` `ultrix31` | full | DEC Ultrix-11 1.0 (= V7M-11 1.0, 1982/83), 2.0 (1984), 3.0 (1986, which merged 4.2BSD TCP/IP as an Executive-mode driver) and 3.1 (1987, the last release) — DEC's productized Version 7 for the PDP-11. The local 3.1 `sysent.c` is authoritative: V7 core numbers (1–48) plus DEC/SysV additions (`msgsys`/`utssys`/`fcntl`/`ulimit`/`nap`… at 49+) and the `UCB_NET` Berkeley socket block (101–119, used by real clients like `rdate`/`tftp`), `0407`/`0410`/`0411` a.out, apsim's Ultrix personality. **Native-validated**: real V7M-11 1.0 `/bin` (`echo`/`cat`/`wc`/`sum`/`ls`/`od`…), carved from the `agn453/V7M-11` disk image into `~/unix/ultrix11/1.0`, runs under `-u ultrix1` and byte-identically across all four, `wc`/`sum` matching the host. The 2.0 install tape carves too (134 native `/bin` from a UNIX `dump`), and those run under `-u ultrix2` — they use Ultrix's stack-argument syscall variant (`sys N|0200`), which apsim decodes; `wc`/`sum` byte-match the host. The 3.0 and 3.1 boot tapes' root dumps carve as well (146 and 145 native `/bin`, `oracle/native/v7dump.py`) and run under `-u ultrix3`/`-u ultrix31` — including their 2.9-derived `ls`, served by the Ultrix "Berkeley block" (the 2.9 local calls flattened onto syscalls 80+: `lstat`=82, `setpgrp`=93, `gethostname`=99, the sockets at 101+…) — the whole DEC Ultrix vector, shared across 1.0–3.1 and correctly dead only under the *non*-Ultrix universes. The boot tapes' optional-software (f77, SCCS, nroff/troff, spell, games) extracts and runs too. (Old names `ultrix11v1`/`ultrix11v2`/`ultrix11` remain as aliases.) |
