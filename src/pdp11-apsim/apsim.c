...
static int
univ_apply(const char *name)
{
	int i;
	for (i = 0; UnivAlias[i].alias; i++)
		if (!strcmp(name, UnivAlias[i].alias)) {
			name = UnivAlias[i].canon;
			break;
		}
...
errno_h2g(int e)
{
	...
	case EDEADLK:
		return modern ? 11 : 5;
	case ELOOP:
		return modern ? 62 : ENOENT;
	case ENAMETOOLONG:
		return modern ? 63 : ENOENT;
	case ENOTEMPTY:
		return modern ? 66 : EEXIST;
	case EDQUOT:
		return modern ? 69 : ENOSPC;
	case ESTALE:
		return modern ? 70 : EIO;
	case ENOSYS:
		return modern ? 78 : EINVAL;
	...
	case EINPROGRESS:
		return modern ? 36 : EINTR;
	case EALREADY:
		return modern ? 37 : EBUSY;
	case ENOTSOCK:
		return modern ? 38 : EINVAL;
	case EDESTADDRREQ:
		return modern ? 39 : EINVAL;
	case EMSGSIZE:
		return modern ? 40 : EINVAL;
	case EPROTOTYPE:
		return modern ? 41 : EINVAL;
	case ENOPROTOOPT:
		return modern ? 42 : EINVAL;
	case EPROTONOSUPPORT:
		return modern ? 43 : EINVAL;
	case ESOCKTNOSUPPORT:
		return modern ? 44 : EINVAL;
	case EOPNOTSUPP:
		return modern ? 45 : EINVAL;
	case EPFNOSUPPORT:
		return modern ? 46 : EINVAL;
	case EAFNOSUPPORT:
		return modern ? 47 : EINVAL;
	case EADDRINUSE:
		return modern ? 48 : EEXIST;
	case EADDRNOTAVAIL:
		return modern ? 49 : EINVAL;
	case ENETDOWN:
		return modern ? 50 : EIO;
	case ENETUNREACH:
		return modern ? 51 : EIO;
	case ENETRESET:
		return modern ? 52 : EIO;
	case ECONNABORTED:
		return modern ? 53 : EIO;
	case ECONNRESET:
		return modern ? 54 : EIO;
	case ENOBUFS:
		return modern ? 55 : ENOMEM;
	case EISCONN:
		return modern ? 56 : EBUSY;
	case ENOTCONN:
		return modern ? 57 : EINVAL;
	case ESHUTDOWN:
		return modern ? 58 : EINVAL;
	case ETIMEDOUT:
		return modern ? 60 : EINTR;
	case ECONNREFUSED:
		return modern ? 61 : EIO;
	case EHOSTDOWN:
		return modern ? 64 : EIO;
	case EHOSTUNREACH:
		return modern ? 65 : EIO;
	default:
		return (e >= 1 && e <= 34) ? e : EIO;
	}
}
...
static int tty_saved = 0, ttyraw = 0;
...
guest_pid(void)
{
	return g_fakepid ? (g_fakepid & 0x7fff) : (getpid() & 0x7fff);
}
...
sig_g2h(int g)
{
	...
	case 6:
		return SIGABRT; /* IOT */
	case 7:
		return 0; /* EMT -- no Linux equivalent (raised internally) */
	case 8:
		return SIGFPE;
	case 9:
		return SIGKILL;
...
static int
sig_dfl_action(int s)
{
	if (s == 17 || s == 18 || s == 21 || s == 22)
		return 2; /* STOP TSTP TTIN TTOU */
	if (s == 16 || s == 19 || s == 20 || s == 23 || s == 28)
		return 1; /* URG CONT CHLD IO WINCH */
	return 0;
}
...
pid_g2h(int g)
{
	...
	if (g == 0)
		return 0;
	for (i = 0; i < 64; i++)
		if (pidmap[i].h && pidmap[i].g == (g & 0x7fff))
			return pidmap[i].h;
	return g;
}
...
ke11_rd(int a)
{
	switch (a & ~1) {
	case 0177302:
		return KAC & 0xffff; /* AC */
	case 0177304:
		return KMQ & 0xffff; /* MQ */
	default:
		return 0; /* DIV/MUL/SC/SR/NOR/LSH/ASH read as 0 */
	}
}
...
ke11_wr(int a, int v)
{
	...
	case 0177302:
		...
		return; /* AC */
	case 0177304:
		KMQ = v & 0xffff; /* MQ: sign extends into AC */
		KAC = (KMQ & 0100000) ? 0xffff : 0;
		return;
	case 0177306: /* MUL: MQ*v -> AC:MQ */
		...
		return;
	default:
		return; /* SC/SR/NOR/LSH/ASH: unused by the corpus */
	}
}
...
{
	a &= 0xffff;
	if (v1sys && a >= KE11LO && a <= KE11HI + 1)
		return ke11_rd(a);
	return M[a] | (M[(a + 1) & 0xffff] << 8);
}

static unsigned short pcring[16];
static int pcri;
static int steppc;
...
st2(int a, int v)
{
	...
	if (a == watchaddr || a + 1 == watchaddr)
		fprintf(DBGF, "apsim: WATCH addr=%06o val=%06o steppc=%06o\n", a, v & 0xffff, steppc);
	if (watchval && (v & 0xffff) >= watchval && (v & 0xffff) <= watchval + 6)
		fprintf(DBGF, "apsim: VALSET addr=%06o val=%06o steppc=%06o r2=%06o\n", a, v & 0xffff, steppc, R[2]);
	if (watchtext && a < tsizew) {
		int k;
		...
		fprintf(DBGF, "  recent pcs:");
		for (k = 0; k < 16; k++)
			fprintf(DBGF, " %06o", pcring[(pcri + k) & 15]);
		fprintf(DBGF, "\n");
		if (--watchtext == 0) {
			halted = 1;
			ecode = 126;
...
st1(int a, int v)
{
	...
	if (a == watchaddr) {
		fprintf(DBGF, "apsim: WATCH1 addr=%06o val=%03o steppc=%06o r2=%06o iob[r2]: ptr=%06o cnt=%06o base=%06o flag=%06o\n", a, v & 0xff, steppc, R[2], ld2(R[2]), ld2(R[2] + 2), ld2(R[2] + 4), ld2(R[2] + 6));
	}
	if (watchtext && a < tsizew) {
		fprintf(DBGF, "apsim: TEXT WRITE1 addr=%06o val=%03o steppc=%06o instr=%06o\n", a, v & 0xff, steppc, ld2(steppc));
		if (--watchtext == 0) {
			halted = 1;
			ecode = 126;
		}
...
static struct fpv
bits2fpv(unsigned long long bits)
{
	struct fpv r;
	int e = (bits >> 55) & 0377;
	if (e == 0) {
...
fpv2bits(struct fpv v)
{
	...
	e = v.exp + 128;
	if (e <= 0)
		return 0; /* underflow -> DEC zero */
	if (e > 0377)
		e = 0377; /* overflow -> clamp */
	return ((unsigned long long)(v.sign & 1) << 63) | ((unsigned long long)e << 55) | (v.frac & 0x7FFFFFFFFFFFFFULL);
}

static struct fpv
rdfloat(int addr, int dbl)
{
	unsigned long long bits = 0;
	int i, nw = dbl ? 4 : 2;
	for (i = 0; i < nw; i++)
		bits = (bits << 16) | ld2((addr + 2 * i) & 0xffff);
	if (!dbl)
		bits <<= 32; /* left-justify F into 64 bits */
	return bits2fpv(bits);
}

static void
wrfloat(int addr, struct fpv v, int dbl)
{
	unsigned long long bits = fpv2bits(v);
	int i, nw = dbl ? 4 : 2;
	for (i = 0; i < nw; i++)
		st2((addr + 2 * i) & 0xffff, (bits >> (16 * (3 - i))) & 0xFFFF);
}
...
static struct fpv
fp_norm128(int sign, int exp, u128fp v, int bits)
{
	struct fpv r;
	unsigned long long frac;
	int sh;
...
static struct fpv
fp_addv(struct fpv a, struct fpv b)
{
	u128fp ma, mb;
	int d;
	if (a.frac == 0)
...
static struct fpv
fp_mulv(struct fpv a, struct fpv b)
{
...
static struct fpv
fp_divv(struct fpv a, struct fpv b)
{
	if (a.frac == 0)
		return fp_zero();
	if (b.frac == 0)
		return fp_zero(); /* div by zero: FP11 traps; give 0 */
	return fp_norm128(a.sign ^ b.sign, a.exp - b.exp,
			  (((u128fp)a.frac << 64) / b.frac) << 56, fpwidth());
}

static int
fp_cmpv(struct fpv a, struct fpv b)
{ /* sign of (a-b), exact */
	if (a.frac == 0 && b.frac == 0)
		return 0;
	if (a.frac == 0)
		...
		return a.sign ? -1 : 1;
	{
		...
		if (a.sign)
			m = -1;
		if (a.exp != b.exp)
			return (a.exp > b.exp) ? m : -m;
		if (a.frac != b.frac)
			return (a.frac > b.frac) ? m : -m;
		return 0;
	}
}
...
static long
fp_tolong(struct fpv v)
{ /* truncate toward zero */
	unsigned long long iv;
	if (v.frac == 0 || v.exp <= 0)
		return 0;
	if (v.exp >= 56)
		iv = v.frac << (v.exp - 56 > 8 ? 8 : v.exp - 56); /* overflow: clamp-ish */
	else
		iv = v.frac >> (56 - v.exp);
...
operand(int spec, int byte)
{
	...
	case 3:
		if (rn == 7) {
			...
			PC = (PC + 2) & 0xffff;
			return a;
		} /* *$abs (addr word I-space; data D-space) */
		a = ld2(R[rn]);
		R[rn] = (R[rn] + 2) & 0xffff;
		return a; /* @(Rn)+  (pointer in D-space) */
	case 4:
		R[rn] = (R[rn] - inc) & 0xffff;
		return R[rn]; /* -(Rn)    */
	case 5:
		R[rn] = (R[rn] - 2) & 0xffff;
		return ld2(R[rn]); /* @-(Rn)  (pointer in D-space) */
	case 6:
		x = ifetch(PC);
		PC = (PC + 2) & 0xffff;
		return (R[rn] + x) & 0xffff; /* X(Rn)  (X from I-space) */
	case 7:
		x = ifetch(PC);
		PC = (PC + 2) & 0xffff;
		return ld2((R[rn] + x) & 0xffff); /* @X(Rn) (X from I; ptr in D) */
	}
	return 0;
}

static int
getv(int loc, int byte)
{
	if (loc & ISIMM) {
		return byte ? (immv & 0xff) : (immv & 0xffff);
	}
	if (loc & ISREG) {
		int v = R[loc & 7];
...
static void
putv(int loc, int v, int byte)
{
	if (loc & ISIMM)
		return; /* a literal can't be a destination */
	if (loc & ISREG) {
		if (byte)
			R[loc & 7] = (R[loc & 7] & 0xff00) | (v & 0xff);
		else
			R[loc & 7] = v & 0xffff;
		return;
	}
...
sgn(int v, int byte)
{
	return byte ? (signed char)v : (signed short)v;
}

static void
setNZ(int v, int byte)
{
	FZ = ((byte ? v & 0xff : v & 0xffff) == 0);
	FN = (sgn(v, byte) < 0);
}
...
{
	st1(buf + 0, 13);
	st1(buf + 1, 13); /* i/ospeed = B9600 */
	st1(buf + 2, 0177);
	st1(buf + 3, 025); /* erase=DEL, kill=^U */
...
stl(int a, long v)
{
	st2(a, (int)((v >> 16) & 0xffff));
	st2(a + 2, (int)(v & 0xffff));
}

/* V7/2.8/2.9 (and sys3/Ultrix-11): 30 bytes -- 7 shorts, then size and
 * three times as longs. */
static void
put_stat_v7(int sb, struct stat *hs)
{
	st2(sb + 0, hs->st_dev);
	st2(sb + 2, hs->st_ino & 0xffff);
	st2(sb + 4, hs->st_mode);
	st2(sb + 6, hs->st_nlink);
	st2(sb + 8, hs->st_uid);
	st2(sb + 10, hs->st_gid);
...
static void
put_stat_v6(int sb, struct stat *hs)
{
	int k;
	long sz = hs->st_size > 0xffffff ? 0xffffff : (long)hs->st_size;
	int mode = 0100000																	/* IALLOC */
		   | (S_ISDIR(hs->st_mode) ? 040000 : 0) | (S_ISCHR(hs->st_mode) ? 020000 : 0) | (S_ISBLK(hs->st_mode) ? 060000 : 0) | (sz > 4095 ? 010000 : 0) /* ILARG */
		   | (hs->st_mode & 07777);
	st2(sb + 0, hs->st_dev);
	st2(sb + 2, hs->st_ino & 0xffff);
	st2(sb + 4, mode);
	st1(sb + 6, hs->st_nlink & 0xff);
	st1(sb + 7, hs->st_uid & 0xff);
	st1(sb + 8, hs->st_gid & 0xff);
	st1(sb + 9, (int)((sz >> 16) & 0xff));
	st2(sb + 10, (int)(sz & 0xffff));
	for (k = 0; k < 8; k++)
		st2(sb + 12 + 2 * k, 0); /* i_addr[8] */
...
static void
put_stat_211(int sb, struct stat *hs)
{
	st2(sb + 0, hs->st_dev);
	st2(sb + 2, hs->st_ino & 0xffff);
	st2(sb + 4, hs->st_mode);
	...
	stl(sb + 14, (long)(hs->st_size > 0xffffff ? 0xffffff : hs->st_size));
	stl(sb + 18, stat_time(hs->st_atime));
	st2(sb + 22, 0); /* int spare1 */
	stl(sb + 24, stat_time(hs->st_mtime));
	st2(sb + 28, 0); /* int spare2 */
	stl(sb + 30, stat_time(hs->st_ctime));
	st2(sb + 34, 0);    /* int spare3 */
...
static void
put_stat(int sb, struct stat *hs, int stat211)
{
	if (stat211)
		put_stat_211(sb, hs);
	else if (Kern == PDP11_K_V56)
		put_stat_v6(sb, hs);
	else
		put_stat_v7(sb, hs);
}
...
dir_build(const char *hpath, int *lenp)
{
	DIR *d;
	struct dirent *e;
	unsigned char *buf;
	int cap = 4096, len = 0, lastrec = -1;
	if (!(d = opendir(hpath)))
		return 0;
	buf = malloc(cap);
	while ((e = readdir(d))) {
		int ino = (int)(e->d_ino & 0xffff);
		if (ino == 0)
			ino = 1; /* 0 = free slot */
		while (len + 600 > cap) {
			cap *= 2;
		...
		if (Kern == PDP11_K_V1) { /* 10-byte: ino + name[8] */
			int k;
			buf[len] = ino & 0xff;
			buf[len + 1] = (ino >> 8) & 0xff;
			for (k = 0; k < 8; k++)
				buf[len + 2 + k] = k < (int)strlen(e->d_name) ? e->d_name[k] : 0;
			len += 10;
		}
			...
			if (rl + 10 > left) {
				buf[len] = 0;
				buf[len + 1] = 0; /* empty pad record */
				buf[len + 2] = left & 0xff;
				buf[len + 3] = (left >> 8) & 0xff;
				buf[len + 4] = 0;
				buf[len + 5] = 0;
...
static void
dir_snapshot(int fd, const char *hpath)
{
	struct gdir *g = 0;
	int i, len;
	unsigned char *buf;
	...
	if (!g)
		return; /* too many open dirs: reads give EISDIR */
	if (!(buf = dir_build(hpath, &len)))
		return;
	g->fd = fd;
	g->buf = buf;
	g->len = len;
...
static long
dir_guest_size(const char *hpath)
{
	int len;
	unsigned char *buf = dir_build(hpath, &len);
	if (!buf)
		return 0;
	free(buf);
	return len;
...
struct sremap {
	unsigned short guest, canon, flags;
};
...
static socklen_t
sockaddr_g2h(int gaddr, int glen, struct sockaddr_storage *hs)
{
...
static void
sockaddr_h2g(struct sockaddr *ha, int gaddr, int glenaddr)
{
	int glen = 16, k;
	if (gaddr == 0) {
		if (glenaddr)
	...
	if (ha->sa_family == AF_INET) {
		...
		const char *p = guestify(su->sun_path);
		int n = strlen(p);
		st2(gaddr, AF_UNIX);
		for (k = 0; k < n && k < 108; k++)
			st1((gaddr + 2 + k) & 0xffff, p[k]);
...
static int
sockopt_g2h_level(int lv)
{
	return lv == G_SOL_SOCKET ? SOL_SOCKET : lv;
}
...
static const struct sremap Bsd210Remap[] = {
	RS_(38, 18), RS_(40, C_LSTAT), RS_(62, 28), {57, C_SYMLINK, 0}, {58, C_READLINK, 0}, {64, C_GETPAGESIZE, 0}, {69, C_SBRK, 0}, {79, C_GETGROUPS, 0}, {80, C_OK, 0}, {81, C_GETPGRP, 0}, {82, C_SETPGRP, 0}, {83, C_OK, 0}, {84, 7, 0}, {86, C_OK, 0}, {87, C_GETHOSTNAME, 0}, {88, C_OK, 0}, {89, C_GETDTABLESIZE, 0}, {90, C_DUP2, 0}, {92, C_FCNTL, 0}, {93, C_SELECT, 0}, {95, C_FSYNC, 0}, {96, C_OK, 0}, {100, C_OK, 0}, {103, C_SIGRETURN, 0}, {108, C_SIGVEC, 0}, {109, C_SIGBLOCK, 0}, {110, C_SIGSETMASK, 0}, {111, 29, 0}, {112, C_OK, 0}, {116, C_GETTIMEOFDAY, 0}, {117, C_GETRUSAGE, 0}, {120, C_READV, 0}, {121, C_WRITEV, 0}, {122, C_OK, 0}, {123, C_OK, 0}, {124, C_FCHMOD, 0}, {128, C_RENAME, 0}, {129, C_TRUNCATE, 0}, {130, C_FTRUNCATE, 0}, {131, C_OK, 0}, {136, C_MKDIR, 0}, {137, C_RMDIR, 0}, {138, C_UTIMES, 0}, {144, C_GETRLIMIT, 0}, {145, C_OK, 0}, {146, C_KILLPG, 0}, {66, 2, 0} /* vfork -> fork (2.11 has this; 2.10 needs it too) */, {76, C_OK, 0} /* vhangup: no controlling tty to revoke */, {91, C_NOSYS, 0}, {94, C_OK, 0} /* getdopt refused; setdopt accepted */,
	...
	{125, C_RECVFROM, 0},
	{126, C_OK, 0},
	{127, C_OK, 0} /* setre[ug]id: accept (identity model) */,
	{133, C_SENDTO, 0},
	{134, C_SHUTDOWN, 0},
	{135, C_SOCKETPAIR, 0},
	{140, C_NOSYS, 0},
...
static const struct sremap Bsd211Remap[] = {
	{7, C_WAIT4, 0}, {13, C_FCHDIR, 0}, {17, C_OK, 0}, {18, C_OK, 0}, {23, C_SYSCTL, 0}, {25, C_GETEUID, 0}, {27, C_GETPPID, 0}, {28, C_STATFS, 0}, {29, C_FSTATFS, 0}, {30, C_GETFSSTAT, 0} /* statfs/fstatfs/getfsstat */, {31, C_SIGVEC, 0} /* sigaction: same record-the-handler treatment */, {32, C_OK, 0}, {34, C_OK, 0}, {35, C_OK, 0}, RS_(38, 18), {39, C_GETLOGIN, 0}, RS_(40, C_LSTAT), {43, C_OK, 0}, {45, 23, 0} /* setuid */, {46, C_OK, 0} /* seteuid */, {48, C_GETEGID, 0}, {49, 46, 0} /* setgid */, {50, C_OK, 0}, {56, C_SIGWAIT, 0}, {57, C_SYMLINK, 0}, {58, C_READLINK, 0}, RS_(62, 28), {64, C_GETPAGESIZE, 0}, {65, C_SELECT, 0} /* pselect -> select */, {66, 2, 0} /* vfork -> fork */, {69, C_SBRK, 0}, {79, C_GETGROUPS, 0}, {80, C_OK, 0}, {81, C_GETPGRP, 0}, {82, C_SETPGRP, 0}, {83, C_OK, 0}, {86, C_OK, 0}, {87, C_GETHOSTNAME, 0}, {88, C_OK, 0}, {89, C_GETDTABLESIZE, 0}, {90, C_DUP2, 0}, {92, C_FCNTL, 0}, {93, C_SELECT, 0}, {95, C_FSYNC, 0}, {96, C_OK, 0}, {100, C_OK, 0}, {103, C_SIGRETURN, 0}, {107, C_SIGSUSPEND, 0} /* pl431 sigsuspend */, {108, C_SIGVEC, 0}, {109, C_SIGBLOCK, 0}, {110, C_SIGSETMASK, 0}, {111, 29, 0}, {112, C_OK, 0}, {116, C_GETTIMEOFDAY, 0}, {117, C_GETRUSAGE, 0}, {120, C_READV, 0}, {121, C_WRITEV, 0}, {122, C_OK, 0}, {123, C_OK, 0}, {124, C_FCHMOD, 0}, {128, C_RENAME, 0}, {129, C_TRUNCATE, 0}, {130, C_FTRUNCATE, 0}, {131, C_OK, 0}, {136, C_MKDIR, 0}, {137, C_RMDIR, 0}, {138, C_UTIMES, 0}, {144, C_GETRLIMIT, 0}, {145, C_OK, 0}, {146, C_KILLPG, 0}, {148, C_OK, 0},
	...
	{153, C_NOSYS, 0},
	{154, C_NOSYS, 0},
	{152, C_OK, 0} /* nostk: no segment to release */,
...
static const struct sremap Sys3Remap[] = {
	{57, C_UNAME, 0} /* utssys */, {62, C_FCNTL, 0}, {63, C_ULIMIT, 0}, {64, C_OK, 0}, {68, C_OK, 0}, {69, C_OK, 0}, {71, C_NAP, 0}, {79, C_NOSYS, 0} /* semsys */, {0, 0, 0}};
...
static const struct sremap Ultrix3Remap[] = {
	{80, C_NOSYS, 0}, {81, C_NOSYS, 0} /* login */, {82, C_LSTAT, 0} /* UCB_SYMLINKS lstat, V7 stat shape */, {83, C_NOSYS, 0} /* submit */, {84, C_OK, 0} /* nostk */, {85, C_NOSYS, 0} /* killbkg */, {86, C_NOSYS, 0} /* killpg (nosys in 3.1) */, {87, C_OK, 0} /* renice: accepted, no scheduler */, {88, C_NOSYS, 0} /* fetchi */, {89, C_NOSYS, 0} /* ucall */, {90, C_NOSYS, 0} /* quota */, {91, C_NOSYS, 0} /* qfstat */, {92, C_NOSYS, 0} /* qstat */, {93, C_SETPGRP, 0}, {94, C_NOSYS, 0} /* gldav (nosys in 3.1) */, {95, C_OK, 0} /* fperr: no pending FP fault to report */, {96, C_NOSYS, 0} /* vhangup (nosys in 3.1) */, {98, C_SELECT, 0}, {99, C_GETHOSTNAME, 0}, {100, C_OK, 0} /* sethostname */,
	...
	{106, C_SOCKETPAIR, 0},
	{107, C_OK, 0},
	{108, C_OK, 0} /* setre[ug]id: identity model */,
	{109, C_SYMLINK, 0},
	{110, C_READLINK, 0},
	{111, C_OK, 0},
	{112, C_OK, 0} /* get/sethostid */,
	{113, C_SETSOCKOPT, 0},
	{114, C_GETSOCKOPT, 0},
	{115, C_GETSOCKNAME, 0},
	{116, C_GETPEERNAME, 0},
...
static int
sremap_apply(const struct sremap *t, int code, int *stat211)
{
	for (; t->guest; t++)
		if (t->guest == code) {
			if (t->flags & SR_STAT)
...
v56_nosys(int n)
{
	if (n == 27 || n == 29 || n == 33 || n == 39 || n == 40 || n == 45)
		return 1;
	if (n >= 49 && n <= 63)
		return 1;
	if (n == 26 && Kern == PDP11_K_V56 && !strcmp(Univ, "v5"))
		return 1; /* v5: no ptrace */
	return 0;
}
...
static void
pt_addr(struct sockaddr_un *a, int pid)
{
	memset(a, 0, sizeof *a);
...
pt_readu(int off)
{
	int k;
	if (off >= 0 && off <= 14)
		return R[off / 2]; /* probe convention */
	if (off == 16)
		return (FC) | (FV << 1) | (FZ << 2) | (FN << 3);
	for (k = 0; k < 8; k++)
		if (off == pt_regoff[k])
			return R[k];
	if (off == PT_PSOFF)
		return (FC) | (FV << 1) | (FZ << 2) | (FN << 3) | 0340; /* +kernel-set bits */
	return 0;
}

static void
pt_writeu(int off, int val)
{
	int k;
	if (off >= 0 && off <= 14) {
		R[off / 2] = val & 0xffff;
...
pt_stop(int sig)
{
	...
	if (pt_cfd < 0) { /* first stop: wait for the tracer to attach */
		...
		if (pt_cfd < 0) {
			pt_traced = 0;
			return sig;
		} /* attach failed: give up tracing */
	}
	notify[0] = 1;
	notify[1] = sig;
	if ((int)write(pt_cfd, notify, sizeof notify) != (int)sizeof notify) {
		pt_traced = 0;
		return sig;
	}
	for (;;) {
		if (read(pt_cfd, msg, sizeof msg) != (int)sizeof msg) {
			pt_traced = 0;
			return 0;
		}
		{
			...
			switch (req) {
			case PT_READ_I:
				rep[1] = Isp[addr] | (Isp[(addr + 1) & 0xffff] << 8);
				break;
			case PT_READ_D:
				rep[1] = M[addr] | (M[(addr + 1) & 0xffff] << 8);
				break;
			case PT_READ_U:
				rep[1] = pt_readu(msg[1]);
				break;
			case PT_WRITE_I:
				Isp[addr] = data & 0xff;
				Isp[(addr + 1) & 0xffff] = (data >> 8) & 0xff;
				break;
			case PT_WRITE_D:
				M[addr] = data & 0xff;
				M[(addr + 1) & 0xffff] = (data >> 8) & 0xff;
				break;
			case PT_WRITE_U:
				pt_writeu(msg[1], data);
				break;
			case PT_CONTINUE:
				if ((msg[1] & 0xffff) != 1)
					PC = addr;
...
static void
pt_notify_exit(int code)
{
	int n[2] = {2, code & 0xff};
	if (pt_cfd >= 0) {
		if (write(pt_cfd, n, sizeof n)) {
		}
	}
}
...
pt_find_tracee(void)
{
	...
	for (k = 0; k < 100; k++) { /* ~100ms: a real tracee connects in <1ms;
		...
		for (i = 0; i < pt_nkids; i++) {
			...
			if (connect(fd, (struct sockaddr *)&a, pt_alen(&a)) == 0) {
				pt_cfd = fd;
				pt_tracee_pid = pt_kids[i] & 0x7fff;
				pt_tracee_hpid = pt_kids[i];
				return 1;
			}
			close(fd);
		}
...
static void
do_syscall(int num, int argaddr)
{
	...
	if (Kern == PDP11_K_BSD210)
		num = sremap_apply(Bsd210Remap, num, &stat211);
	else if (Kern == PDP11_K_BSD211)
		num = sremap_apply(Bsd211Remap, num, &stat211);
		...
		if (systrace)
			fprintf(DBGF, "sys %d: nosys in this era\n", num);
		FC = 1;
		R[0] = EINVAL;
		return;
	}
	...
	if (Kern == PDP11_K_BSD2X && num == 57)
		num = 2; /* 2.9 vfork: treat as fork */
	if (systrace)
		fprintf(DBGF, "[%d] sys %d fd0=%d a1=%06o a2=%06o a3=%06o\n", getpid() & 0x7fff, num, fd0, a1, a2, a3);
	switch (num) {
	case 1: /* exit(code in r0; 2.10: on stack) */
		halted = 1;
		ecode = (stackargs ? a1 : R[0]) & 0xff;
		return;
	...
	case 7: { /* wait: reap a child; r0=pid, r1=status word.
		...
		int st, opts = 0, wpid;
		if (FC && FV && FZ && FN)
			opts = ((R[0] & 1) ? WNOHANG : 0) | ((R[0] & 2) ? WUNTRACED : 0);
		if (pt_enabled && pt_find_tracee()) { /* a traced child parked on a trap:
			...
			if (read(pt_cfd, n, sizeof n) == (int)sizeof n) {
				if (n[0] == 1) {
					R[1] = ((n[1] << 8) | 0177) & 0xffff;
				} /* WIFSTOPPED */
				else {
					R[1] = ((n[1] & 0xff) << 8) & 0xffff; /* WIFEXITED */
	...
	case 5: { /* open(a1=path, a2=mode) */
		...
		struct stat ds;
		if (systrace)
			fprintf(DBGF, "    open ret=%06o path='%s'\n", ld2(SP), (char *)(M + (a1 & 0xffff)));
		strncpy(hp, mappath(a1), sizeof hp - 1);
		hp[sizeof hp - 1] = 0;
	...
	case 8: /* creat(a1=path, a2=mode) */
		if (systrace)
			fprintf(DBGF, "    creat path='%s' mode=%o\n", mappath(a1), a2);
	...
	case 19: { /* V5/V6: seek(fd, offset16, ptrname 0-5);
		 ...
		 * return truncates ftell() and archive scans. */
		{
			...
			if (g) {
				g->pos = (whence == 0) ? off : (whence == 1) ? g->pos + off
									     : g->len + off;
				if (g->pos < 0)
					g->pos = 0;
				R[1] = g->pos & 0xffff;
	...
	case 11: { /* execv(a1=name, a2=argv): replace the image and
		...
		while (n < 64) {
			...
			if (p == 0)
				break;
			strncpy(argbuf[n], (char *)(M + (p & 0xffff)), 255);
			argbuf[n][255] = 0;
			args[n] = argbuf[n];
			n++;
		}
		if (load_aout(hp, n, args) < 0) {
			FC = 1;
			R[0] = ENOENT;
			break;
		}
	...
	case C_LSTAT: {
		...
		if (num == 28) {
			...
			if (systrace)
				fprintf(DBGF, "    lstat path='%s' -> %s\n", (char *)(M + (a1 & 0xffff)), mappath(a1));
			ok = lstat(mappath(a1), &hs);
			sb = a2 & 0xffff;
		}
		else {
			if (systrace)
				fprintf(DBGF, "    stat path='%s'\n", mappath(a1));
			ok = stat(mappath(a1), &hs);
			sb = a2 & 0xffff;
		}
		if (ok < 0) {
			r = -1;
		...
		if (S_ISDIR(hs.st_mode)) {
			...
			if (g)
				...
				hs.st_size = dir_guest_size(num == 28 ? "." : mappath(a1));
		}
	...
	case 54: {	      /* ioctl(fd, request, argp).  Inline form (V7..2.9):
		...
		default:
			r = 0;
			break; /* accept other tty ioctls quietly */
		}
		break;
	}
	case 48: { /* signal(a1=sig, a2=handler): record the guest
		...
		hs = sig_g2h(sig);
		if ((sig >= 4 && sig <= 12) || sig == SIGSTOP || hs == 0)
			; /* fault/STOP: record only */
		else if (h == 1)
			signal(hs, SIG_IGN);
		else if (h == 0)
			signal(hs, SIG_DFL);
	...
	case 23: /* setuid(uid in r0): sandbox allows it; set r&e uid */
		g_ruid = g_euid = R[0] & 0xffff;
		r = 0;
		break;
	case 46: /* setgid(gid in r0): set r&e gid */
		g_rgid = g_egid = R[0] & 0xffff;
		r = 0;
		break;
	...
	{
		char p1[1024];
		strncpy(p1, mappath(a1), sizeof p1 - 1);
		p1[sizeof p1 - 1] = 0;
	...
	case 35: /* V5/V6: sleep(sec in r0): really sleep.
		...
		}
		{
			...
			if (e)
				t = atol(e);
			stl(a1 & 0xffff, t);
			st2((a1 + 4) & 0xffff, 0); /* millitm */
			st2((a1 + 6) & 0xffff, 0);
			st2((a1 + 8) & 0xffff, 0); /* tz, dst */
			r = 0;
		}
		break;
	...
	case 26: { /* ptrace(req, pid, addr, data): the cooperative
		...
		}
		{
			...
			if (write(pt_cfd, msg, sizeof msg) != (int)sizeof msg ||
			    read(pt_cfd, rep, sizeof rep) != (int)sizeof rep) {
				errno = ESRCH;
				r = -1;
				break;
			}
	...
	case 52: /* phys: map physical memory (privileged) */
		errno = EPERM;
		r = -1;
		break; /* unsupported, fail cleanly */
	...
	case 31: /* stty(fd=r0, &sgttyb=a1): old-style tty set,
		...
		tty_apply(ld2((a1 & 0xffff) + 4) & 0xffff);
		r = 0;
		break;
	...
	case 59: { /* exece(path, argv, envp): exec with an explicit
		...
		static char ab[64][256], eb[64][256];
		char *av[64], *ev[64];
		char hp[1024];
		...
		while (na < 63) {
			...
			if (!p)
				break;
			strncpy(ab[na], (char *)(M + (p & 0xffff)), 255);
			ab[na][255] = 0;
			av[na] = ab[na];
			na++;
		}
		while (ep && ne < 63) {
			...
			if (!p)
				break;
			strncpy(eb[ne], (char *)(M + (p & 0xffff)), 255);
			eb[ne][255] = 0;
			ev[ne] = eb[ne];
			ne++;
		}
		if (load_aout_env(hp, na, av, ne, ev) < 0) {
			FC = 1;
			R[0] = ENOENT;
			break;
		}
	...
	case 58: { /* local(sub): 2.8's site-syscall dispatcher.
		...
		if (sub == 14) { /* gldav(av): short av[3] at R0, all zero */
			...
			r = 0; /* other local calls: succeed, no effect */
		break;
	}
	...
	case C_SIGVEC: { /* 4.3-style sigvec(108)/sigaction(31).  The
		...
		if (vec) {
			...
			hs2 = sig_g2h(sig);
			if ((sig >= 4 && sig <= 12) || hs2 == 0)
				; /* fault sigs: record only */
			else if (h == 1)
				signal(hs2, SIG_IGN);
			else if (h == 0)
				signal(hs2, SIG_DFL);
	...
	case C_WAIT4: { /* wait4(pid, *status, options, *rusage):
		...
		if ((int)(short)a1 < 0 && a1 != 0xffff)
			who = -who; /* negative = pgrp */
		...
		if (a2) {
			int gst;
			if (WIFSTOPPED(st))
				gst = ((sig_h2g(WSTOPSIG(st)) << 8) | 0177);
			else if (WIFEXITED(st))
				gst = (WEXITSTATUS(st) & 0xff) << 8;
			else
				gst = sig_h2g(WTERMSIG(st)) & 0x7f;
			st2(a2 & 0xffff, gst & 0xffff);
		}
		r = wpid & 0x7fff;
		break;
	...
	case C_GETPAGESIZE:
		r = 1024;
		break; /* 2.11 ctob(CLSIZE) */
	case C_GETDTABLESIZE:
		r = 30;
		break; /* NOFILE */
	case C_DUP2:
		r = dup2(fd0, a1);
	...
	case C_SETPGRP: /* setpgrp(pid, pgrp) -> host setpgid; this
			 ...
			 * and signals to it. */
		r = setpgid(pid_g2h(a1), pid_g2h(a2));
		break;
	...
	case C_SELECT: { /* select(nfds, *r, *w, *e, *tv): guest fds are
		...
		if (ta) {
			long sec = ((long)(ld2(ta & 0xffff) & 0xffff) << 16) | (ld2((ta + 2) & 0xffff) & 0xffff);
	...
	case C_READV:
	case C_WRITEV: { /* readv/writev(fd, *iov, cnt) */
		int iov = a1 & 0xffff, cnt = a2 & 0xffff, k;
		long tot = 0;
		if (cnt > 16)
			cnt = 16;
		r = 0;
	...
	case C_RENAME: {
		...
		strncpy(p1, mappath(a1), sizeof p1 - 1);
		p1[sizeof p1 - 1] = 0;
		r = rename(p1, mappath(a2));
		break;
	}
	case C_SYMLINK: {
		char p1[1024];
		strncpy(p1, (char *)(M + (a1 & 0xffff)), sizeof p1 - 1);
		p1[sizeof p1 - 1] = 0;
		r = symlink(p1, mappath(a2));
		break; /* target string verbatim */
	}
	case C_READLINK: {
		char lb[1024];
		int n2, b = a2 & 0xffff, max = a3 & 0xffff, k;
	...
	case C_KILLPG: { /* killpg(pgrp, sig): real host process group */
		...
		r = hs ? killpg(pid_g2h(a1), hs) : 0;
		break;
	}
	case C_UNAME: { /* sys3/Ultrix utssys(buf, 0, 0): 5 x 9-char
			 * fields sysname/node/release/version/machine */
		int b = a1 & 0xffff, k;
		const char *f[5] = {"apsim", "apsim", "3", "1", "pdp11"};
		for (k = 0; k < 5; k++) {
			int j;
			for (j = 0; j < 9; j++)
				st1(b + 9 * k + j, j < (int)strlen(f[k]) ? f[k][j] : 0);
		}
		r = 0;
		break;
	}
	...
	case C_SYSCTL: { /* 2.11 pl431 __sysctl(name, namelen, old,
		...
		if (top == 1) { /* CTL_KERN */
			if (sub == 1)
				s = "2.11BSD"; /* KERN_OSTYPE */
			else if (sub == 2)
				s = "2.11BSD"; /* KERN_OSRELEASE */
			else if (sub == 4)
				s = "2.11 BSD UNIX (apsim)"; /* KERN_VERSION */
			else if (sub == 10)
				s = "apsim"; /* KERN_HOSTNAME */
		}
	...
	case C_SOCKET: /* socket(domain, type, protocol) */
		...
		break;
	case C_BIND:
	case C_CONNECT: { /* (fd0=fd, a1=*addr, a2=addrlen) */
	...
	case C_ACCEPT: { /* accept(fd0, a1=*addr, a2=*addrlen) -> newfd */
		...
		if (r >= 0 && a1)
			sockaddr_h2g((struct sockaddr *)&ss, a1 & 0xffff, a2 & 0xffff);
		break;
	}
	...
	case C_GETSOCKNAME:
	case C_GETPEERNAME: { /* (fd0, a1=*addr, a2=*addrlen) */
		struct sockaddr_storage ss;
		socklen_t hl = sizeof ss;
	...
	case C_GETSOCKOPT: { /* getsockopt(fd0, a1=level, a2=name, val, *len) */
		...
		if (r >= 0 && vptr)
			st2(vptr, hostval & 0xffff); /* int-sized options */
		if (r >= 0 && lptr)
			st2(lptr, 2);
		break;
	}
	...
	case C_SOCKETPAIR: { /* socketpair(a1=dom, a2=type, a3=proto, sv[2]) */
		...
		if (r >= 0) {
			st2(svp, sv[0]);
			st2((svp + 2) & 0xffff, sv[1]);
		}
		break;
	}
	case C_SENDMSG:
	case C_RECVMSG: { /* (fd0=fd, a1=*msghdr, a2=flags).
		...
		struct msghdr mh;
		struct iovec hv;
		static unsigned char mbuf[65536];
		long tot = 0;
		int k;
		if (iovn > 1024)
			iovn = 1024;
		for (k = 0; k < iovn; k++) {
			int l = ld2((iov + 4 * k + 2) & 0xffff);
			tot += l;
		}
		if (tot > (long)sizeof mbuf)
			tot = sizeof mbuf;
		...
		mh.msg_iovlen = 1;
		if (name) {
			sl = sockaddr_g2h(name, namelen, &ss);
			mh.msg_name = &ss;
			mh.msg_namelen = sl;
		}
		if (num == C_SENDMSG) { /* gather guest iov -> mbuf */
			...
			if (r >= 0) {
				...
				if (name)
					sockaddr_h2g((struct sockaddr *)&ss, name, mp + 2);
	...
	case C_STATFS:
	case C_FSTATFS: { /* 2.11 statfs/fstatfs -> host statvfs */
		struct statvfs vs;
		int ok, sb;
		const char *on = "/";
		if (num == C_FSTATFS) {
	...
	case C_SIGWAIT: { /* sigwait(*set, *sig): block until a signal in
		...
		int setp = a1 & 0xffff, sigp = a2 & 0xffff;
		long want;
		if (!setp || !sigp) {
			errno = EINVAL;
			r = -1;
...
static void
v1statout(struct stat *hs, int sb)
{
	int fl = 0100000 | 020000; /* used|modified */
	long c = v1sixty(hs->st_ctime), m = v1sixty(hs->st_mtime);
	int ino = hs->st_ino & 0x7fff;
	if (ino < 41)
		ino += 100; /* <41 = device inodes */
	if (hs->st_size > 4095)
		fl |= 010000; /* large file */
	if (S_ISDIR(hs->st_mode))
		fl |= 040000;
	if (hs->st_mode & 04000)
		fl |= V1M_SUID;
	...
	st2(sb + 0, ino);
	st2(sb + 2, fl);
	st1(sb + 4, hs->st_nlink & 0xff);
	st1(sb + 5, hs->st_uid & 0xff);
	...
	st2(sb + 24, (c >> 16) & 0xffff);
	st2(sb + 26, c & 0xffff);
	st2(sb + 28, (m >> 16) & 0xffff);
	st2(sb + 30, m & 0xffff);
	st2(sb + 32, 0);
}

static void
do_v1syscall(int num, int argaddr)
{
	int a1 = ld2(argaddr), a2 = ld2(argaddr + 2);
	long r = 0;
	if (systrace)
		fprintf(DBGF, "v1sys %d r0=%06o a1=%06o a2=%06o\n", num, R[0], a1, a2);
	switch (num) {
	case 0:	 /* rele: give up the processor */
	case 17: /* break: grow the data area (flat here) */
	case 26:
	case 27: /* quit / intr: trap-catch switches */
	case 29:
	case 33: /* cemt / ilgins */
	case 34: /* nice */
		FC = 0;
	...
	case 25:
	case 31:
	case 32: /* stime / stty / gtty: succeed */
		FC = 0;
		return;
	...
	case 35: { /* V2 sleep: suspend for r0/60 seconds */
		...
		struct timespec ts;
		ts.tv_sec = t60 / 60;
		ts.tv_nsec = (long)(t60 % 60) * (1000000000L / 60);
		nanosleep(&ts, 0);
		FC = 0;
		return;
	}
	...
	case 21:
	case 22: /* mount / umount */
		FC = 1;
		return;
	case 1: /* exit: NEITHER V1 nor V2 exit reliably
		 * carries a status -- r0 is whatever the
		 * last syscall left (surveyed: every
		 * native v2 /bin binary returns leftover
		 * junk here, e.g. ld=255, dc=134), and
		 * even the s2 tape's `as' doesn't set it.
		 * So report 0.  $APSIM_V1EXIT=r0 opts in
		 * to reading r0 for the few 1972 binaries
		 * (and our own compiled programs) that do
		 * set it.  (An earlier v2-defaults-to-r0
		 * experiment was reverted: it gave native
		 * v2 binaries garbage exit codes.) */
		halted = 1;
		{
	...
	case 5: { /* open(name, mode 0/1/2) */
		int hm = a2 == 0 ? O_RDONLY : a2 == 1 ? O_WRONLY
						      : O_RDWR;
	...
	case 9: { /* link(old, new) */
		char oldp[1024];
		strncpy(oldp, mappath(a1), sizeof oldp - 1);
		oldp[sizeof oldp - 1] = 0;
		FC = link(oldp, mappath(a2)) < 0;
		return;
	}
	case 10: /* unlink(name) */
		FC = unlink(mappath(a1)) < 0;
		return;
	case 11: { /* exec(name, argp): argp -> ptrs, 0-end */
		static char argbuf[32][256];
		char *args[32];
		char hp[1024];
		int n = 0, aptr = a2 & 0xffff, p;
		strncpy(hp, mappath(a1), sizeof hp - 1);
	...
	case 14: /* makdir(name, mode) */
		FC = mkdir(mappath(a1), v1mode_to_host(a2) | 0700) < 0;
		return;
	case 15: /* chmod(name, mode) */
		FC = chmod(mappath(a1), v1mode_to_host(a2)) < 0;
		return;
	case 16: /* chown(name, owner) */
		FC = chown(mappath(a1), a2 & 0x3fff, -1) < 0;
		return;
	case 18: { /* stat(name; buf) */
		struct stat hs;
		if (stat(mappath(a1), &hs) < 0) {
			FC = 1;
			return;
		}
		v1statout(&hs, a2 & 0xffff);
		FC = 0;
		return;
	}
...
do_v1sys(int instr)
{
	...
	if (num > limit) {
		fprintf(DBGF, "apsim: bad V1 sys %o at pc=%06o\n", num, (PC - 2) & 0xffff);
		halted = 1;
		ecode = 127;
		return;
	}
...
{ /* branch taken? */
	...
	case 0002400:
		return (FN ^ FV) != 0; /* BLT  */
	case 0003000:
		return (FZ | (FN ^ FV)) == 0; /* BGT  */
	case 0003400:
		return (FZ | (FN ^ FV)) != 0; /* BLE  */
	case 0100000:
		return !FN; /* BPL  */
	case 0100400:
		return FN; /* BMI  */
...
static int
fp_addr(int spec, int dbl)
{
	int mode = (spec >> 3) & 7, rn = spec & 7, size = dbl ? 8 : 4, a;
	if (mode == 2) {
		a = R[rn];
		R[rn] = (R[rn] + size) & 0xffff;
		return a;
	} /* (Rn)+ */
	if (mode == 4) {
		R[rn] = (R[rn] - size) & 0xffff;
...
static struct fpv
fp_get(int spec, int dbl)
{ /* float source operand */
	if (((spec >> 3) & 7) == 0)
		return AC[spec & 7];
...
static void
fp_put(int spec, struct fpv v, int dbl)
{ /* float dest operand */
	if (((spec >> 3) & 7) == 0) {
		AC[spec & 7] = v;
		return;
...
do_fp(int instr)
{
	...
	int pc0 = (PC - 2) & 0xffff;
	if (fptrace < 0)
		fptrace = (getenv("APSIM_FPTRACE") != 0);
	...
	case 0170100:
		FPS = isreg ? R[reg] : getv(operand(spec, 0), 0);
		FPTR("%06o: ldfps 0x%x\n", pc0, FPS);
		return; /* ldfps */
	...
	case 0170200:
		if (isreg)
			...
			putv(operand(spec, 0), FPS, 0);
		return; /* stfps */
	case 0170400:	/* clrf */
		fp_put(spec, fp_zero(), dbl);
		fN = 0;
		fZ = 1;
		fV = 0;
		fC = 0;
	...
	case 0171000:
		sv = fp_get(spec, dbl);
		FPTR("%06o: mulf %f %f", pc0, fpv2d(AC[ac]), fpv2d(sv));
		AC[ac] = fp_mulv(AC[ac], sv);
		fp_setcc(AC[ac]);
		FPTR(" -> %f NZCV %d%d%d%d\n", fpv2d(AC[ac]), fN, fZ, fC, fV);
		return; /* mulf */
	case 0172000:
		sv = fp_get(spec, dbl);
		FPTR("%06o: addf %f %f", pc0, fpv2d(AC[ac]), fpv2d(sv));
		AC[ac] = fp_addv(AC[ac], sv);
		fp_setcc(AC[ac]);
		FPTR(" -> %f NZCV %d%d%d%d\n", fpv2d(AC[ac]), fN, fZ, fC, fV);
		return; /* addf */
	case 0173000:
		sv = fp_get(spec, dbl);
		FPTR("%06o: subf %f %f", pc0, fpv2d(AC[ac]), fpv2d(sv));
		AC[ac] = fp_addv(AC[ac], fp_negv(sv));
		fp_setcc(AC[ac]);
		FPTR(" -> %f NZCV %d%d%d%d\n", fpv2d(AC[ac]), fN, fZ, fC, fV);
		return; /* subf */
	case 0174400:
		sv = fp_get(spec, dbl);
		FPTR("%06o: divf %f %f", pc0, fpv2d(AC[ac]), fpv2d(sv));
		AC[ac] = fp_divv(AC[ac], sv);
		fp_setcc(AC[ac]);
		FPTR(" -> %f NZCV %d%d%d%d\n", fpv2d(AC[ac]), fN, fZ, fC, fV);
		return; /* divf */
	case 0173400:	/* cmpf: CC from (fsrc-AC), compared exactly */
	{
		struct fpv fs = fp_get(spec, dbl);
		int c = fp_cmpv(fs, AC[ac]);
		fN = (c < 0);
		fZ = (c == 0);
		fV = 0;
		fC = 0;
		...
		return;
	case 0172400:
		AC[ac] = fp_get(spec, dbl);
		fp_setcc(AC[ac]);
		FPTR("%06o: ldf %f\n", pc0, fpv2d(AC[ac]));
		return; /* movf LDF */
	case 0174000:
		fp_put(spec, AC[ac], dbl);
		fp_setcc(AC[ac]);
		FPTR("%06o: stf %f\n", pc0, fpv2d(AC[ac]));
		return; /* movf STF */
	case 0177400:
	...
	case 0175000: /* movei (STEXP): dst = unbiased exponent of AC[ac] */
	{
		...
		if (isreg)
			...
			putv(operand(spec, 0), e & 0xffff, 0);
		FPTR("%06o: stexp %f -> %d\n", pc0, fpv2d(AC[ac]), e);
	}
		return;
	...
	case 0177000: /* movif: int -> float (rounds at mode width) */
		if (isreg)
			...
			addr = fpi_addr(spec);
			if (addr & ISIMM)
				iv = (short)immv; /* $literal: one word */
			else
				iv = (FPS & FPL) ? (int)((ld2(addr) << 16) | ld2((addr + 2) & 0xffff))
						 : (short)ld2(addr);
		}
		AC[ac] = fp_fromlong(iv);
		fp_setcc(AC[ac]);
		FPTR("%06o: ldcif int%d %ld -> %f\n", pc0, (FPS & FPL) ? 32 : 16, iv, fpv2d(AC[ac]));
		return;
	case 0175400: /* movfi: float -> int (truncate toward zero) */
		...
		if (isreg)
			...
			addr = fpi_addr(spec);
			if (addr & ISIMM)
				; /* $literal dest: discard */
			else if (FPS & FPL) {
				st2(addr, (iv >> 16) & 0xffff);
				st2((addr + 2) & 0xffff, iv & 0xffff);
			}
			else
				st2(addr, iv & 0xffff);
		}
...
static int
cc_cmp16(int a, int b)
{ /* condition codes of CMP a,b */
	int r = (a - b) & 0xffff;
	FZ = (r == 0);
	FN = (r >> 15) & 1;
...
static u128
dec_read(int d0, int d1, int *neg)
{
	...
	*neg = 0;
	...
	for (i = 0; i < len; i++) {	     /* zoned/overpunch/separate: high digit first */
		...
		if (ty == 0 && i == len - 1 && (c & 0xf0) == 0x70)
			*neg = 1; /* signed zoned: 7X zone */
		v = v * 10 + (dg <= 9 ? dg : 0);
	}
	if (ty == 4 && ld1(a + len) == '-')
		*neg = 1; /* trailing separate */
	if (ty == 5 && ld1(a) == '-')
		*neg = 1; /* leading separate */
	return v;
}

/* decimal write: truncates to the descriptor length; returns 1 on overflow */
static int
dec_write(int d0, int d1, u128 v, int neg)
{
	...
	}
	{
		int off = (ty == 5) ? 1 : 0;
		for (i = 0; i < len; i++)
			st1(a + off + i, 0x30 | dg[len - 1 - i]);
		if (ty == 0 && neg && len)
			st1(a + off + len - 1, 0x70 | dg[0]); /* signed zoned: 7X */
		if (ty == 2 && len) {
			int d = dg[0]; /* trailing overpunch */
			st1(a + len - 1, neg ? (d ? 'J' + d - 1 : '}') : (d ? 'A' + d - 1 : '{'));
		}
		if (ty == 3 && len) {
			int d = dg[len - 1]; /* leading overpunch */
...
cis(int instr)
{
	...
	case 032: { /* movc/movrc/movtc */
		...
		for (i = 0; i < n; i++)
			st1(d1 + i, tmp[i]); /* buffered: overlap-safe */
		for (; i < d0; i++)
			st1(d1 + i, fill);
		cc_cmp16(s0, d0);
	...
	case 044: { /* cmpc */
		...
		if (i == n) {
			...
			FC = (c1 < c2);
			FV = (((c1 ^ c2) & (~c2 ^ r)) >> 7) & 1;
			R[0] = (i < s0) ? (s0 - i) & 0xffff : 0;
			R[1] = (s1 + (i < s0 ? i : s0)) & 0xffff;
			R[2] = (i < d0) ? (d0 - i) & 0xffff : 0;
			R[3] = (d1 + (i < d0 ? i : d0)) & 0xffff;
		}
		return 1;
	}
	...
	case 050:
	case 051:
	case 052: /* addn/subn/cmpn */
	case 070:
	case 071:
	case 072: { /* addp/subp/cmpp */
		int a0, a1, b0, b1, c0, c1, n1, n2, ovf;
		u128 v1, v2;
		__int128 s1v, s2v, r;
		cis_dsc(0, &a0, &a1);
		cis_dsc(2, &b0, &b1);
		v1 = dec_read(a0, a1, &n1);
		v2 = dec_read(b0, b1, &n2);
	...
	case 054:
	case 055: { /* cvtpn/cvtnp */
		int a0, a1, c0, c1, n1, ovf;
		u128 v;
		cis_dsc(0, &a0, &a1);
		cis_dsc(2, &c0, &c1);
		v = dec_read(a0, a1, &n1);
...
late_insn(int instr)
{
	...
	if ((instr & 0177770) == 0000230)
		return 1;		    /* spl: user-mode no-op */
	if ((instr & 0177700) == 0007200) { /* tstset */
		int d, old;
		if ((instr & 070) == 0)
			return 0; /* mode 0: reserved */
		d = operand(instr & 077, 0);
		old = getv(d, 0);
		R[0] = old;
		putv(d, old | 1, 0);
		setNZ(old, 0);
	...
	if ((instr & 0177740) == 0075000) { /* fis (KEV11): stack F-format
		...
		case 0:
			...
			break;
		case 1:
			res = fp_addv(a, fp_negv(b));
			break;
		case 2:
			res = fp_mulv(a, b);
			break;
		default:
			res = (b.frac == 0) ? fp_zero() : fp_divv(a, b);
			break; /* div0: lenient */
		}
		wrfloat(addr, res, 0);
		FPS = savefps;
		R[r] = addr;
		FN = (res.frac != 0) && res.sign;
		FZ = (res.frac == 0);
		FV = 0;
		FC = 0;
		return 1;
	...
	if ((instr & 0177600) == 0076000 && (instr & 0177) != 0100) /* cis */
		return cis(instr);
	if (instr == 1)
		return 1; /* wait: user-mode no-op */
	if (instr == 5)
		return 1;		    /* reset: user-mode no-op */
...
step(void)
{
	...
	if (((instr & 0177400) >= 0000400 && (instr & 0177400) <= 0003400) ||
		...
		if (c >= 0) {
			if (c) {
				int off = (signed char)(instr & 0377);
				PC = (PC + 2 * off) & 0xffff;
			}
			return;
		}
	...
	if ((instr & 0177400) == 0104000) { /* EMT (0104000|n) */
		...
		if (ov_proc) { /* 0430/0431: overlay switch, ovno in r0 (csv.s
			...
			if (n >= 1 && n <= 15 && ov_siz[n] > 0) {
				...
				cur_ovno = n;
				if (systrace)
					fprintf(DBGF, "apsim: overlay -> %d (%d bytes @%06o%s)\n", n, ov_siz[n], ov_base, ov_sep ? " I" : "");
			}
			else {
	...
	if ((op >= 1 && op <= 6) || (op >= 011 && op <= 016)) {
		...
		case 2:
			...
			setNZ(r, byte);
			{
				int m = byte ? 0xff : 0xffff, sb = byte ? 0x80 : 0x8000;
				FC = ((sv & m) < (dv & m));
				FV = (((sv ^ dv) & (~dv ^ r)) & sb) != 0;
			}
			break;
	...
	if (op == 07) {
		...
		case 0070000: /* MUL: R*src -> R,R|1 (32-bit) */
			s = operand(instr & 077, 0);
			sv = (short)getv(s, 0);
			{
				long p = (long)(short)R[reg] * sv;
				R[reg] = (p >> 16) & 0xffff;
				R[reg | 1] = p & 0xffff;
				FZ = (p == 0);
				FN = (p < 0);
		...
		case 0073000: /* ASHC: the pair is a SIGNED 32-bit value; right
			...
			sv = (short)getv(s, 0);
			{
				...
				R[reg] = ((unsigned long)val >> 16) & 0xffff;
				R[reg | 1] = val & 0xffff;
				FZ = (val == 0);
				FN = (val < 0);
				FV = 0;
	...
	/* single-operand (word/byte): 0?05DDDD..0?063DDD, JMP, SWAB, SXT */
	{
		...
		case 0005200:
			...
			putv(d, r, b);
			setNZ(r, b); /* INC */
			FV = ((b ? (dv & 0xff) == 0x7f : (dv & 0xffff) == 0x7fff));
			return;
		case 0005300:
			d = operand(spec, b);
			dv = getv(d, b);
			...
			putv(d, r, b);
			setNZ(r, b); /* DEC */
			FV = ((b ? (dv & 0xff) == 0x80 : (dv & 0xffff) == 0x8000));
			return;
		case 0005400:
			d = operand(spec, b);
			dv = getv(d, b);
		...
		case 0006000:
			...
			dv = getv(d, b); /* ROR */
			{
				int m = b ? 0xff : 0xffff, nb = b ? 0x80 : 0x8000;
				r = (dv >> 1) | (FC ? nb : 0);
				FC = dv & 1;
				putv(d, r, b);
				setNZ(r, b);
	...
	case 0000003: /* BPT -> SIGTRAP (adb breakpoints) */
		if (pt_traced) {
			pt_pending = 5;
			return;
		} /* traced: trap-stop for the tracer */
		if (raise_fault(5))
			return;
	...
	if ((instr & 0177400) == 0170000)
		return; /* FP control (setd/setf/seti/setl/cfcc...) */
	if ((instr & 0177000) >= 0170000)
		return; /* other FP: ignore */
	...
	{
		int a;
		fprintf(DBGF, "  arena window 0111130..0111154:");
		for (a = 0111130; a <= 0111154; a += 2)
			fprintf(DBGF, " %06o:%06o", a, ld2(a));
		fprintf(DBGF, "\n");
		fprintf(DBGF, "  recent pcs:");
		{
			int k;
...
static char *
mappath_s(const char *gp)
{
	static char buf[1024];
	char *root = getenv("APSIM_ROOT");
	if (root && gp[0] == '/')
		snprintf(buf, sizeof buf, "%s%s", root, gp);
	else
...
static void
put_statfs(int sb, struct statvfs *vs, const char *on)
{
	int k;
	long u = vs->f_frsize / 1024;
	if (u < 1)
		u = 1;
	for (k = 0; k < 232; k++)
		st1((sb + k) & 0xffff, 0);
	st2(sb + 0, 1);
	st2(sb + 2, 0); /* f_type=UFS, f_flags */
	st2(sb + 4, 1024);
	st2(sb + 6, 1024); /* f_bsize, f_iosize */
	stl(sb + 8, (long)(vs->f_blocks * u));
	stl(sb + 12, (long)(vs->f_bfree * u));
	stl(sb + 16, (long)(vs->f_bavail * u));
	st2(sb + 20, vs->f_files & 0xffff);
	st2(sb + 22, vs->f_ffree & 0xffff);
	{
		const char *p = guestify(on);
		int n = strlen(p);
		for (k = 0; k < n && k < 89; k++)
			st1((sb + 52 + k) & 0xffff, p[k]);
	}
	{
		const char *f = "apsim";
		for (k = 0; f[k]; k++)
			st1((sb + 142 + k) & 0xffff, f[k]);
	}
}
...
static void
setup_stack(int nargs, char **args, int nenv, char **env)
{
	static char envbuf[512];
	char *envv[32];
	int k, straddr = 0177700, sa;
	if (env) {
		for (k = 0; k < nenv && k < 32; k++)
			envv[k] = env[k];
		if (nenv > 32)
			nenv = 32;
	}
	else {
		char *e = getenv("APSIM_ENV");
		char *p;
	...
	for (k = 0; k < nargs; k++)
		straddr -= strlen(args[k]) + 1;
	for (k = 0; k < nenv; k++)
		straddr -= strlen(envv[k]) + 1;
	straddr &= ~1;
	...
	for (k = 0; k < nargs; k++) {
		st2(SP + 2 + 2 * k, sa);
		strcpy((char *)M + sa, args[k]);
		sa += strlen(args[k]) + 1;
	}
	st2(SP + 2 + 2 * nargs, 0); /* argv NULL */
	for (k = 0; k < nenv; k++) {
		st2(SP + 4 + 2 * nargs + 2 * k, sa);
		strcpy((char *)M + sa, envv[k]);
		sa += strlen(envv[k]) + 1;
	}
...
static void
setup_stack_v1(int nargs, char **args)
{
	...
	for (k = nargs - 1; k >= 0; k--) {
		...
		posn -= len;
		memcpy(M + posn, args[k], len);
		aposn[k] = posn;
	}
	posn &= ~1;
	posn -= 2;
	st2(posn, 0177777); /* -1 ends the arg vector */
	for (k = nargs - 1; k >= 0; k--) {
		posn -= 2;
		st2(posn, aposn[k]);
	}
...
static int
load_script(FILE *f, const char *path, int use_line,
	    int nargs, char **args, int nenv, char **env)
{
	...
	if (use_line) { /* parse "#! interp [arg]" */
		...
		for (p = q + strlen(q); p > q && (p[-1] == ' ' || p[-1] == '\t'); p--)
			;
		if (p > q)
			snprintf(iarg, sizeof iarg, "%.*s", (int)(p - q), q);
	}
	else
...
static void
loadseg(unsigned char *space, int off, int count, FILE *f)
{
	off &= 0xffff;
	if (count < 0)
		count = 0;
	if (off + count > 0x10000)
		count = 0x10000 - off;
	if (count > 0) {
		if (fread(space + off, 1, count, f)) {
		}
	}
}

static int
load_aout_env(const char *path, int nargs, char **args, int nenv, char **env)
{
	FILE *f = fopen(path, "rb");
	int hdr[8], i, nread, tsize, dsize, entry;
	if (!f)
		return -1;
	...
	if (hdr[0] == 0430 || hdr[0] == 0431) {
		...
		if (ov_sep) {
			...
			for (j = 1; j <= novl; j++)
				if (ov_siz[j] > 0) {
					if (fread(ov_img[j - 1], 1, ov_siz[j], f)) {
					}
				}
			loadseg(M, 0, dsize, f);
		...
		if (getenv("APSIM_DBASE"))
			dbase = (int)strtol(getenv("APSIM_DBASE"), 0, 8);
		tsizew = (hdr[0] == 0407) ? 0 : tsize;
		Isp = M;
		memset(M, 0, 1 << 16);
		loadseg(M, 0, tsize, f);
		loadseg(M, dbase, dsize, f);
...
static int
load_aout(const char *path, int nargs, char **args)
{
...
{
	FILE *f;
	int hdr[8], ai = 1, tsize, dsize, bsize, entry;
	long long i;
	 ...
	 * Falls back to plain stderr if the dup fails. */
	{
		...
		if (e >= 0)
			Dbg = fdopen(e, "w");
		if (Dbg)
			setvbuf(Dbg, (char *)0, _IONBF, 0);
	}
	/* env first, so a command-line flag can override it */
	if (getenv("APSIM_WATCHTEXT"))
		watchtext = atoi(getenv("APSIM_WATCHTEXT"));
	if (getenv("APSIM_WATCHSP"))
		watchsp = 1;
	if (getenv("APSIM_WATCHADDR"))
		watchaddr = (int)strtol(getenv("APSIM_WATCHADDR"), 0, 8);
	if (getenv("APSIM_WATCHVAL"))
		watchval = (int)strtol(getenv("APSIM_WATCHVAL"), 0, 8);
	if (getenv("APSIM_UID")) {
		g_ruid = g_euid = atoi(getenv("APSIM_UID"));
	}
	if (getenv("APSIM_GID")) {
		g_rgid = g_egid = atoi(getenv("APSIM_GID"));
	}
	if (getenv("APSIM_PID")) {
		g_fakepid = atoi(getenv("APSIM_PID"));
	}
	if (getenv("APSIM_PTRACE"))
		pt_enabled = 1; /* enable the ptrace debug channel */
...
#undef X
	...
	if (getenv("APSIM_SYSARGS") && !strcmp(getenv("APSIM_SYSARGS"), "stack"))
		stackargs = 1;
	...
	while (ai < argc && argv[ai][0] == '-' && argv[ai][1]) {
		...
		else if (!strcmp(argv[ai], "-2")) {
			stackargs = 1;
			ai++;
		} /* stack-arg syscalls override */
		else if (!strcmp(argv[ai], "-p") && ai + 1 < argc) {
			g_fakepid = atoi(argv[ai + 1]);
			ai += 2;
		}
		else if (!strncmp(argv[ai], "--universe=", 11) || !strcmp(argv[ai], "-u") || !strcmp(argv[ai], "--universe")) {
			const char *nm;
			if (argv[ai][1] == '-' && argv[ai][10] == '=') {
				nm = argv[ai] + 11;
				ai++;
			}
			else {
				if (ai + 1 >= argc) {
					fprintf(DBGF, "apsim: %s needs a name\n", argv[ai]);
					return 2;
				}
				nm = argv[ai + 1];
				ai += 2;
...
#undef X
	...
	if (ai >= argc) {
		fprintf(DBGF, "usage: apsim [-t] [-s] [-p pid] [-u universe] [-2] a.out [args]\n");
		return 2;
	}
	...
	for (i = 0; i < 4000000000LL && !halted; i++) {
		if (pt_traced && pt_pending) { /* traced: park on the trap and
			...
			if (halted)
				break;
			if (s)
				pending_sig = s; /* tracer re-injected a signal */
		}
		...
		if (pending_sig && !((guest_sigmask >> (pending_sig - 1)) & 1)) {
			...
			if (s > 0 && s < 32 && (h = guest_sigh[s]) > 1 && guest_reliable[s]) {
				...
				case 2:
					raise(SIGSTOP);
					break; /* really stop; CONT resumes */
				case 1:
					break; /* ignore */
				default:
					halted = 1;
					ecode = 128 + s; /* terminated by signal */
				}
				if (halted)
					break;
			}
		}
		if (watchsp && (SP < 0150000 || SP > 0177700)) {
			fprintf(DBGF, "apsim: SP=%06o pc=%06o instr=%06o (i=%lld)\n", SP, PC, ifetch(PC), i);
			halted = 1;
			ecode = 126;
			break;
		}
		steppc = PC;
		pcring[pcri = (pcri + 1) & 15] = PC;
		if (rndpc == -2) {
			char *e = getenv("APSIM_RNDPC");
			rndpc = e ? (int)strtol(e, 0, 8) : -1;
		}
		if (rndpc >= 0 && PC == rndpc)
			fprintf(DBGF, "RND %d r=%d s=%06o ret=%06o\n", rndn++, ld2((R[6] + 2) & 0xffff), ld2(041202), ld2(R[6]));
		if (trace)
			fprintf(DBGF, "pc=%06o sp=%06o r0=%06o instr=%06o\n", PC, SP, R[0], ifetch(PC));
		if ((i & 0xFFFFFFF) == 0xFFFFFFF)
			fprintf(DBGF, "apsim: %lldM pc=%06o sp=%06o\n", i >> 20, PC, SP);
		step();
		if (pt_traced && pt_step)
			pt_pending = 5; /* single-step done -> SIGTRAP stop */
	}
	if (pt_traced)
		pt_notify_exit(ecode); /* let the tracer's wait see us go */
	if (!halted) {
		fprintf(DBGF, "apsim: instruction limit\n");
		return 125;
	}
	return ecode;
}
