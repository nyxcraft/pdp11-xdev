...
getincdir(char *av0)
{
	...
	if (last[-3] == 'b' && last[-2] == 'i' && last[-1] == 'n' && last[0] == '/') {
		...
		/* era headers live in include/<universe>/ (see universe.h);
		 * fall back to a flat include/ if the subdir is absent */
		univ = getenv("PDP11_UNIVERSE");
		if (univ == 0 || *univ == 0)
			univ = PDP11_UNIV_DEFAULT_NAME;
		if (strlen(incdir) + strlen(univ) + 2 < sizeof incdir) {
			strcat(incdir, "/");
			strcat(incdir, univ);
			if (stat(incdir, &sb) == 0)
				return incdir;
			incdir[strlen(incdir) - strlen(univ) - 1] = '\0';
		}
		return incdir;
	}
	return 0;
...
main(int argc, char **argv)
{
			...
			if (fin == STDIN) {
				...
				*/
				...
				if (NULL == (fout = fopen(argv[i], "w"))) {
					...
					fclose(stdout);
					setvbuf(fout, _sobuf, _IOFBF, sizeof _sobuf);
				}
#endif
			}
			else
	 ...
	 * (.../usr/bin/<p>-cpp -> .../usr/include); 2.9BSD dropped 2.8's UCB_PATH */
	{
		char *ic = getincdir(argv[0]);
		if (ic)
			dirs[nd++] = ic;
	}
