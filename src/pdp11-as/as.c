...
static int kwskip(struct op *o){
	if(isa==1972){
		static char *v4new[]={".ascii","ldfps","stfps","movie","movei",0};
		int i;
		if(o->type==035||o->type==036) return 1;	/* jbr/jxx */
		for(i=0;v4new[i];i++) if(!strcmp(o->name,v4new[i])) return 1;
		/* V4 also added FPS access and the exponent idiom -- the
		 * 1972 table has the FP group WITHOUT these four */
	}
	if(sysnames && !strcmp(o->name,"wait")) return 1;	/* the syscall
	...
	if(isa==211){
		static char *eae[]={"ac","mq","sc","sr","csw","mpy","dvd",
				    "nor","lsh","als","alsc",0};
		int i;
		for(i=0;eae[i];i++) if(!strcmp(o->name,eae[i])) return 1;
	}
	return 0;
}
...
static struct op *oplook(char *name){ struct op *o;
	...
	if(sysnames==6) for(o=v6systab;o->name;o++) if(strcmp(o->name,name)==0) return o;
	if(jflag) for(o=extoptab;o->name;o++) if(strcmp(o->name,name)==0) return o;
	return 0; }
...
static struct sym *lookup(char *name) {
	...
	sp->onext=0; if(symtail)symtail->onext=sp; else symhead=sp; symtail=sp;
	return sp;
}
...
static struct sym *tildesym(char *name){
	...
	memcpy(sp->name,nb,NCPS); sp->seg=SEXT; sp->index=-1;
	sp->onext=0; if(symtail)symtail->onext=sp; else symhead=sp; symtail=sp;
	if(ntilde<MAXTILDE) tildesyms[ntilde]=sp;
	ntilde++; tildeidx++;
	return sp;
}
...
again:
	...
	if (c=='_'||c=='.'||c=='~'||isalpha(c)) {
		...
		if(!toklocal){
			for(o=optab;o->name;o++) if(strcmp(o->name,tokname)==0 && !kwskip(o)) tokkw=o;
			if((isa==211||jflag) && !tokkw)
				for(o=tab211;o->name;o++) if(strcmp(o->name,tokname)==0) tokkw=o;
			if(sysnames==1972 && !tokkw)
				for(o=v1systab;o->name;o++) if(strcmp(o->name,tokname)==0) tokkw=o;
			if(sysnames==6 && !tokkw)
				for(o=v6systab;o->name;o++) if(strcmp(o->name,tokname)==0) tokkw=o;
			if(jflag && !tokkw)
				for(o=extoptab;o->name;o++) if(strcmp(o->name,tokname)==0) tokkw=o;
		}
		return tok=TID;
	}
...
static long term(int *segp) {
	...
	if(t==TID){
		...
		if(strcmp(tokname,"..")==0){ *segp=SABS; return dotdot; }	/* reloc base constant */
		...
		if(tokkw){ *segp=SABS; return tokkw->opcode; }
		sp=lookup(tokname);
		*segp=SEXT; exsym=sp; return 0;
	}
...
juxta:;
			...
			if(t=='+' && peek()=='('){ pushtok('+',0,0,0); break; }
			v2=term(&seg2); reg=0; reskw=0;
			switch(t){
			...
			case '^': reskw=termkw; seg=seg2; break;	/* the RESULT takes the
...
static void emitword(int w, int seg, struct sym *sym, int pcrel)
{
	...
	if(pass==2){
		...
		else if(seg==SEXT && sym) rel = (sym->index<<4)|REXT|(pcrel?RPCREL:0);
...
static void emitextra(int xval,int xseg,struct sym *xsym,int pcrel)
{
	...
	if(xseg==STEXT||xseg==SDATA||xseg==SBSS) v += dotdot;
	if(pcrel){
...
static void getop(struct operand *o)
{
	...
	{ int rr=-1;
	  if(t==TID && s_kw && s_kw->type==024) rr=s_kw->opcode;	/* r0..r5/sp/pc */
		...
		if(sp && (sp->flags&(SF_DEF|SF_REG))==(SF_DEF|SF_REG)) rr=sp->value&7;
	  }
	  if(rr>=0 && peek()!='('){ o->mode=(defer?1:0)*010+rr; return; }	/* Rn ; *Rn=(Rn) */
	}
	...
	{ int seg; long v; struct sym *sym;
	  ...
	  if(peek()=='+') lex();	/* c1's `disp+(reg)' index separator (expr() stopped here) */
	  if(peek()=='('){
		lex(); t=lex(); reg=regof(); if(reg<0){aerror("bad register");reg=0;}
		if(lex()!=')')aerror("missing )");
...
static int spanrec(struct operand *d){
	...
	if(sidx<MAXSPAN){
		...
		if(!shortok) spanlong[sidx]=1;
			...
			if(d->xsym) spanlong[sidx]=1;
			else { int disp=d->xval-dot[curseg]; spanlong[sidx]=(disp>=0 || disp < -254); }
		}
		else if(deciding){
...
static void doword(long v,int seg,struct sym*sym){
	...
	if(seg==STEXT||seg==SDATA||seg==SBSS) v += dotdot;
...
static void assemble()
{
	...
	for(;;){
		...
		if(ifoff && !(t==TID && tokkw && (tokkw->type==021||tokkw->type==022))){
			...
			if(t==TID && !tokkw && !toklocal) lookup(tokname);
			while(peek()!=TNL && peek()!=TEOF){ t=lex(); if(t==TID && !tokkw && !toklocal) lookup(tokname); }
			continue;
		}
		...
		if(t==TID){
			...
			int nmloc;
			strcpy(name,tokname); kw=tokkw; nmloc=toklocal;
			...
			if(t2==':'){
				if(strcmp(name,".")!=0){
					...
					if(sp->defpass==passno && !nmloc) aerror("redefined symbol");
					sp->defpass=passno;
					sp->flags|=SF_DEF; sp->seg=curseg+1;
			...
			if(t2=='='){
				int seg; long v; struct sym *sp=0;
				...
				v=expr(&seg);
				if(strcmp(name,"..")==0){ dotdot=v; continue; }	/* reloc base, not a symbol */
				if(strcmp(name,".")==0){
				...
				} else { sp->flags|=SF_DEF; sp->seg=seg; sp->value=v;
			...
			if(kw){ struct sym*us=find_sym(name);
...
static void writeout()
{
	...
	for(sp=symhead;sp;sp=sp->onext){
		if(emitsym(sp)){ sp->index=idx++; nsym++; }
	}
	...
	if(v1fmt){
		...
		memcpy(body,segbuf[0],tsize); memcpy(body+tsize,segbuf[1],dsize);
		for(w=0;w<nw;w++){
			unsigned char *rb = (2*w<tsize)? relbuf[0]+2*w : relbuf[1]+2*w-tsize;
			int r=rb[0]|(rb[1]<<8), code=-1, ext=-1;
			if((r&016)==0) code=0;			/* absolute (incl abs-pcrel) */
			else if((r&016)!=REXT) code=(r&1)?0:1;	/* internal: pcrel is
								 * position-independent */
			else{	/* external: word := symtab offset, addend -> extension */
				int idx=r>>4, ad=body[2*w]|(body[2*w+1]<<8);
				body[2*w]=(12*idx)&0xff; body[2*w+1]=(12*idx)>>8;
				if(r&1) code = ad? 014 : 2;	/* 1100 / 10 */
				else    code = ad? 016 : 015;	/* 1110 / 1101 */
				if(ad) ext=ad;
			}
			{ int nb = (code<2)?2 : (code==2)?2 : 4, k;
			  int bits = (code<2)? code : (code==2)? 2 : code;
			  for(k=nb-1;k>=0;k--){
				int bv=(bits>>k)&1;
				if(bv) stream[(bit/16)*2 + 1-(bit%16)/8] |= 0x80>>((bit%8));
				bit++;
			  }
			  if(ext>=0) for(k=15;k>=0;k--){
				if((ext>>k)&1) stream[(bit/16)*2 + 1-(bit%16)/8] |= 0x80>>((bit%8));
				bit++;
			  }
			}
		}
		{ int rsz=((bit+15)/16)*2;
		  ssize=nsym*12;
		  putw_(f,0405); putw_(f,12+tsize+dsize); putw_(f,ssize);
		  putw_(f,rsz); putw_(f,bsize); putw_(f,0);
		  fwrite(body,1,tsize+dsize,f);
		  for(sp=symhead;sp;sp=sp->onext){
			int nt,v1t;
			if(!emitsym(sp)) continue;
			if(sp->flags&SF_REG) nt=N_REG;
			else if(sp->flags&SF_DEF){ switch(sp->seg){case STEXT:nt=N_TEXT;break;
				case SDATA:nt=N_DATA;break;case SBSS:nt=N_BSS;break;default:nt=N_ABS;} }
			else nt=N_UNDF;
			v1t = nt==N_UNDF?0 : nt==N_ABS?1 : nt==N_REG?2 : 3;
			if((sp->flags&SF_GLOBL) || (!(sp->flags&SF_DEF) && !v7flag)) v1t|=040;
		...
		fclose(f); free(body); free(stream);
		return;
	}
	...
	for(sp=symhead;sp;sp=sp->onext){
		...
		if(!emitsym(sp)) continue;
		if(sp->kwtype) nt=sp->kwtype;	/* emitted `^' alias of a non-opcode: n_type is the
						 * carried opcode type (`error=104000^sys' -> 011), not N_ABS */
		else if(sp->flags&SF_REG) nt=N_REG;	/* `rer = r3' -> register name (024), as 2BSD as emits */
		else if(sp->flags&SF_DEF){ switch(sp->seg){case STEXT:nt=N_TEXT;break;case SDATA:nt=N_DATA;break;
		...
		if((sp->flags&SF_GLOBL) || (!(sp->flags&SF_DEF) && !ovasflag && !v7flag)) nt|=N_EXT;
		if(newsym){
			...
			putw_(f,(strx>>16)&0xffff); putw_(f,strx&0xffff);
	...
	if(newsym){
		...
		long tl=strx;
		putw_(f,(tl>>16)&0xffff); putw_(f,tl&0xffff);
		for(sp=symhead;sp;sp=sp->onext){
			if(!emitsym(sp)) continue;
...
int main(int argc, char **argv)
{
		...
		for(i=1;i<argc;i++){
			if(argv[i][0]=='-'){
				if(argv[i][1]=='-' && !strncmp(argv[i],"--isa=",6)){
					...
					char *t=argv[i]+6;
					if(!strcmp(t,"v4")||!strcmp(t,"common")||!strcmp(t,"unix")) isa=0;
					else if(!strcmp(t,"v1")||!strcmp(t,"v2")||!strcmp(t,"v3")||!strcmp(t,"1972")) isa=1972;
					else if(!strcmp(t,"bsd211")||!strcmp(t,"newbsd")||!strcmp(t,"211")) isa=211;
					else if(!strcmp(t,"extended")) jflag=1;
					else{ fprintf(stderr,"as: --isa= takes v1,v4,bsd211,extended\n"); return 1; }
				}
				else if(argv[i][1]=='-' && !strncmp(argv[i],"--sys=",6)){
					char *t=argv[i]+6;
					if(!strcmp(t,"none")) sysnames=0;
					else if(!strcmp(t,"v1")||!strcmp(t,"1972")) sysnames=1972;
					else if(!strcmp(t,"v6")) sysnames=6;
					else{ fprintf(stderr,"as: --sys= takes none,v1,v6\n"); return 1; }
				}
					 ...
					 *        its own, hence the `+' */
					char *t=argv[i]+7;
					if(!strcmp(t,"v1")||!strcmp(t,"405")) v1fmt=1;
					else if(!strcmp(t,"v2")||!strcmp(t,"407")){ v1fmt=0; newsym=0; }
					else if(!strcmp(t,"v2+")||!strcmp(t,"newsym")||!strcmp(t,"bsd211")){ newsym=1; ncps=32; }
					else{ fprintf(stderr,"as: --aout= takes v1,v2,v2+\n"); return 1; }
				}
					...
					while(*t){
						...
						if(!strcmp(tok,"v1")||!strcmp(tok,"v2")||!strcmp(tok,"v3")){
							isa=1972; sysnames=1972; v7flag=1; understood=1;
							if(tok[1]=='1') v1fmt=1; }
						if(!strcmp(tok,"v4")||!strcmp(tok,"v5")||!strcmp(tok,"v6")){
							isa=0; sysnames=6; v7flag=1; understood=1; }
						if(!strcmp(tok,"v7")||!strcmp(tok,"bsd")){
							isa=0; sysnames=0; v7flag=1; understood=1; }
						if(!strcmp(tok,"newbsd")){
							isa=211; sysnames=0; v7flag=1; newsym=1; ncps=32; understood=1; }
						if(!strcmp(tok,"extended")){ jflag=1; understood=1; }
						if(!understood){
				...
				else if(argv[i][1]=='o' && i+1<argc) outfile=argv[++i];
				else if(argv[i][1]=='u') uflag=1; /* undefined->external.  In the
				...
				else if(argv[i][1]=='V') ovasflag=1;	/* ovas mode */
				else if(argv[i][1]=='n'){ newsym=1; ncps=32; }	/* 2.11 string-table format */
				else if(argv[i][1]=='j') jflag=1;	/* late-hardware mnemonics */
				else if(argv[i][1]=='7') v7flag=1;	/* the genuine no-'-u'
		...
		for(i=0;i<nf;i++){
			if(!strcmp(files[i],"--")){		/* standard input */
				...
				while((got=fread(p+len,1,cap-len,stdin))>0){
					len+=got;
					if(len==cap){ char *np=xalloc(cap*2);
						memcpy(np,p,len); p=np; cap*=2; }
				}
				files[i]=p; flens[i]=len; total += len + 1;
				continue;
			}
	...
	relaxing=1;
	{
	  ...
	  lineno=1; pbsp=0; memset(loccnt,0,sizeof loccnt); spanidx=0; tildeidx=0;
	  assemble();
	  nspan=spanidx;
	  estimating=0;
	...
	{ struct sym *sp; int idx=0;
	  for(sp=symhead;sp;sp=sp->onext)
		if(emitsym(sp))
			sp->index=idx++;
	}
	/* pass 2: emit, using the relaxed span sizes */
	passno++; pass=2; ip=ibufstart; curseg=0; dot[0]=dot[1]=dot[2]=0; lineno=1; pbsp=0; memset(loccnt,0,sizeof loccnt); spanidx=0; tildeidx=0;
	assemble();
