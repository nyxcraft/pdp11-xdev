...
build(op)
{
	...
	case SEQNC:
		 ...
		 * p2's subsp/strp -- otherwise a struct-/array-typed sequence node
		 * has a NULL strp and length()/treeout later deref it.  Harmless on
		 * the PDP-11 (reads address 0); a SIGSEGV on an LP64 host. */
		t = t2;
		*cp++ = block(op, t, p2->subsp, p2->strp, p1, p2);
		return;
