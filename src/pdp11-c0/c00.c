...
loop:
	...
	case SHARP:
		...
		if (peekc == '"') {
			sp = filename;
			while ((c = mapch('"')) >= 0)
				/* host paths can exceed the vintage 64-byte
				 * buffer; truncate instead of overflowing
				 * into adjacent globals */
				if (sp < &filename[sizeof filename - 1])
					*sp++ = c;
			*sp++ = 0;
			peekc = getchar();
		}
 ...
 */
/*
 * cmst -- the operand stack tree() builds an expression on, indexed by the
 * global cp.  In the original PDP-11 c0 this was a tree()-local array; callers
 * (doret, initlist, the statement handlers) go on using cp AFTER tree() returns,
 * writing more operands into cmst and only then calling build()/rcexpr().  That
 * relies on the array outliving tree()'s frame -- fine on the PDP-11's stack
 * discipline, but on a modern LP64 host the returned frame is reused by the very
 * build()/rcexpr() calls and the dangling `*cp++' writes smash the stack (c0
 * SIGSEGVs in doret on the first large function, e.g. sh/cmd.c).  cp is already a
 * single global, so tree() is effectively non-reentrant w.r.t. it; give cmst the
 * same file scope so it lives in BSS and cp never dangles.  On-disk output and
 * the CMSIZ overflow check are unchanged.
 */
...
opon1:
	...
	if (p > ps || p == ps && (opdope[o] & RASSOC) != 0) {
		...
		if (initflg) {
			if ((o == COMMA && *op != LPARN && *op != CALL) || (o == COLON && *op != QUEST)) {
				p = 00;
				goto opon1;
			}
		}
